...
Install the facial-recognition Python dependencies:
...
The script first runs `python3 -m access_control.facial_recognition.src.hailo.diagnostics`. If
...
```bash
bash edge/facial_recognition/diagnose_hailort_stack.sh
```
...
If unset, the code uses `edge/facial_recognition/data/device_local.db`.
...
```bash
python edge/facial_recognition/setup_sqlite.py --database /path/to/device_local.db
```
...
export EDGE_SYNC_LOG_PUSH_INTERVAL_SECONDS=60
...
```bash
python3 -m access_control.facial_recognition.src.pipelines.access_control --no-audio
```
...
```bash
python3 -m access_control.facial_recognition.src.pipelines.run_both --camera /dev/video2 --no-display
```
...
`edge/facial_recognition/models` plus `edge/facial_recognition/data`.
...
```bash
mkdir -p edge/facial_recognition/vendor
# Copy the HailoRT Python wheel into edge/facial_recognition/vendor first.
export EDGE_HAILORT_WHEEL=edge/facial_recognition/vendor/hailort-<version>-cp311-<platform>.whl
docker compose -f edge/facial_recognition/docker/docker-compose.face.yml up --build
```
...
```bash
python -m access_control.facial_recognition.src.hailo.diagnostics
```

To run the same check manually after the container starts:

```bash
docker compose -f edge/facial_recognition/docker/docker-compose.face.yml exec edge-face \
  python -m access_control.facial_recognition.src.hailo.diagnostics
```
...
```bash
EDGE_REQUIRE_HAILORT=0 docker compose -f edge/facial_recognition/docker/docker-compose.face.yml up --build
```
