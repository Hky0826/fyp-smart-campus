...
class EdgeAuthToken:
    access_token:str;session_id:int|None;user_id:int;email:str|None=None;roles:tuple[str,...]=();expires_at:datetime|None=None
    @classmethod
    def from_payload(cls,payload:dict[str,Any]):return cls(str(payload['access_token']),int(payload['session_id']) if payload.get('session_id') is not None else None,int(payload['user_id']),str(payload['email']) if payload.get('email') else None,tuple(map(str,payload.get('roles',[]))),_parse(payload.get('expires_at')))
