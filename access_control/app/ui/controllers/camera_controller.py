...
FRAME_INTERVAL_MS = int(os.getenv("ACCESS_GUI_CAMERA_FRAME_INTERVAL_MS", str(round(1000 / max(1, int(os.getenv("ACCESS_DISPLAY_FPS", "24")))))))
...
def _camera_candidates(camera: str) -> list[str | int]:
    ...
    if os.getenv("ACCESS_GUI_CAMERA_NO_FALLBACK", "").strip().lower() in {"1", "true", "yes", "on"}:
        return candidates

    fallback_text = os.getenv("ACCESS_GUI_CAMERA_FALLBACKS", DEFAULT_CAMERA_FALLBACKS)
...
def _configure_capture(capture: Any, source: str | int) -> None:
    if cv2 is None or not isinstance(source, int):
        return
    width = int(os.getenv("ACCESS_GUI_CAMERA_WIDTH", str(DEFAULT_CAMERA_WIDTH)))
    height = int(os.getenv("ACCESS_GUI_CAMERA_HEIGHT", str(DEFAULT_CAMERA_HEIGHT)))
    fourcc_name = os.getenv("ACCESS_GUI_CAMERA_FOURCC", DEFAULT_CAMERA_FOURCC).strip().upper()
    if fourcc_name and fourcc_name not in {"NONE", "DEFAULT", "AUTO"}:
        if len(fourcc_name) != 4:
            raise RuntimeError("ACCESS_GUI_CAMERA_FOURCC must be a 4-character code, or NONE")
    ...
    def run(self) -> None:
        ...
        if capture is None:
            ...
            self.errorChanged.emit(
                ...
                f"{tried}. Check ACCESS_GUI_CAMERA, camera permissions, and whether another process is using the device."
            )
            self.readyChanged.emit(False)
            return
...
class CameraController(QObject):
    ...
    def __init__(self, provider: CameraImageProvider) -> None:
        super().__init__()
        camera = os.getenv("ACCESS_GUI_CAMERA") or os.getenv("ACCESS_CAMERA") or DEFAULT_CAMERA
        self._provider = provider
