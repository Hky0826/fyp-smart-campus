"""Access-control liveness heuristic adapted for OpenCV YuNet landmarks."""
...
class SpoofResult:
    state: str  # "live", "spoof", "unknown"
    score: float
    reason: str


class MotionSpoofDetector:
    """Lightweight motion/pose liveness gate for access control."""
    ...
    def check(self, frame: np.ndarray, face: FaceDetection) -> SpoofResult:
        if cv2 is None:
            return SpoofResult("unknown", 0.0, "opencv_unavailable")

        x1, y1, x2, y2 = face.box.as_xyxy()
        if x2 <= x1 or y2 <= y1:
