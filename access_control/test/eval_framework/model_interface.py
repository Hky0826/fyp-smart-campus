...
from access_control.facial_recognition.src.face.detection import HailoSCRFDDetector
from access_control.facial_recognition.src.face.alignment import FaceAligner
from access_control.facial_recognition.src.face.embedding import HailoArcFaceEmbedder

# Global toggle and parameters for easily testing the degradation logic
# Toggle this to True to enable the camera degradation pipeline by default.
...
class CameraSimulator:
    def __init__(self, enabled=ENABLE_CAMERA_SIMULATION, scale_percent=CAMERA_SCALE_PERCENT, 
                 blur_kernel=CAMERA_BLUR_KERNEL, noise_variance=CAMERA_NOISE_VARIANCE, 
    ...
    def apply(self, img):
        ...
        # 1. Resolution Degradation (downsample then upsample)
        h, w = img.shape[:2]
...
class ModelInterface:
    ...
    def extract_embedding(self, img_path):
        ...
        # Apply camera degradation simulation if enabled
