"""ByteTrack multi-object tracking implementation for person tracking."""
...
class STrack:
    """Individual object track representing a person across video frames."""
...
class ByteTracker:
    """ByteTrack multi-object tracker for preserving person tracks across frames."""
    ...
    def update(self, output_results: List[np.ndarray]) -> List[STrack]:
        ...
        scores = [float(det[4]) for det in output_results] if output_results else []
        ...
        matches, u_t0, u_detection_low = linear_assignment(dists, thresh=0.5)
        ...
        dists = iou_distance(unconfirmed, detections_unconfirmed)
        matches, u_unconfirmed, u_detection_unconfirmed = linear_assignment(dists, thresh=0.7)
        ...
        # Step 4: Init new tracks for unmatched high confidence detections
        for inew in u_detection_unconfirmed:
            track = detections_unconfirmed[inew]
            if track.score >= self.new_track_thresh:
                track.activate(self.kalman_filter, self.frame_id)
                activated_stracks.append(track)

        # Step 5: Update track states & remove expired lost tracks
    ...
    def remove_duplicate_stracks(
        ...
        resb = [t for j, t in enumerate(stracksb) if j not in dupb]
        return resa, resb
