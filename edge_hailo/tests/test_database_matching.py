...
from edge_hailo.src.face.database import DeviceUserRepository
from edge_hailo.src.face.matching import FaceTemplate, TemplateMatcher
...
class DatabaseAndMatchingTests(unittest.TestCase):
    def test_database_loader_reads_multiple_embeddings_from_device_users(self):
        db_path = Path.cwd() / "edge_hailo" / "tests" / "_device_users_test.db"
        if db_path.exists():
            db_path.unlink()
        try:
