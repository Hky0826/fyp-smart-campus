...
The edge audio pipeline calls `POST /api/chatbot/chat/stream` with a JSON body such as `{"query":"Where is the library?","device_id":"entry-gate-01"}`. If it has a face-auth JWT, it also sends `Authorization: Bearer <JWT>` to unlock RBAC levels above visitor/PUBLIC. If it has no JWT, the chatbot can still answer using PUBLIC documents. If the query appears to require protected documents, the response asks the edge user to scan their face.
