"""
Cloud Timing Logger for RAG Chatbot.

Synchronized logging for end-to-end latency and transit timing:
  [CLOUD <- EDGE] : Requests or audio streams received from the Access Control kiosk
  [CLOUD INTERNAL] : Server-side processing milestones (ASR, RAG query, LLM response)
  [CLOUD -> EDGE] : Events, audio chunks, and responses sent back to the kiosk

Writes to cloud/logs/chatbot_timing.log with auto-flush.
"""
...
logger = logging.getLogger("RagChatbot.timing")
...
def get_timing_log_file_path() -> Path:
    """Resolve and ensure the cloud timing log file path."""
...
class CloudTimingLogger:
    """Logger dedicated to tracking chatbot network and pipeline latencies."""
    ...
    def log_event(
        ...
        **details: Any,
        ...
        Returns the server timestamp (ISO string) generated for this event.
        """
        server_ts = now_iso()
        human_time = now_formatted()

        if client_sent_at and transit_delay_ms is None:
            transit_delay_ms = calc_transit_delay_ms(client_sent_at)

        parts = [f"[{human_time}]", f"[{direction}]", f"[{stage}]"]
        if turn_id:
            parts.append(f"turn_id={turn_id}")
        if client_sent_at:
            parts.append(f"client_sent_at={client_sent_at}")
    ...
    def log_internal(
        ...
        **details: Any,
        ...
        """Helper for [CLOUD INTERNAL] events."""
        return cls.log_event(
            "CLOUD INTERNAL",
            stage,
            turn_id=turn_id,
...
cloud_timing = CloudTimingLogger
