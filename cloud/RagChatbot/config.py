...
class RagSettings:
    ...
    # Gemini Live voice output settings (primary audio synthesis)
    LIVE_VOICE_NAME: str = os.getenv(
        "RAG_LIVE_VOICE_NAME", os.getenv("RAG_AUDIO_TTS_VOICE", "Kore")
    )

    # Standalone TTS settings (Deprecated - all audio output is handled by Gemini Live)
    AUDIO_TTS_MODEL: str = os.getenv("RAG_AUDIO_TTS_MODEL", "gemini-3.1-flash-tts-preview")
    AUDIO_TTS_VOICE: str = LIVE_VOICE_NAME
    AUDIO_TTS_FALLBACK_MODELS: str = os.getenv("RAG_AUDIO_TTS_FALLBACK_MODELS", "")
    ...
    # Standalone cloud TTS is disabled; all voice is generated by Gemini Live.
    AUDIO_TTS_ENABLED: bool = os.getenv(
        "RAG_AUDIO_TTS_ENABLED", "false"
    ).strip().lower() in {"1", "true", "yes", "on"}
    AUDIO_TTS_TIMEOUT_SECONDS: float = float(os.getenv("RAG_AUDIO_TTS_TIMEOUT_SECONDS", "10"))
    AUDIO_TTS_WORKERS: int = int(os.getenv("RAG_AUDIO_TTS_WORKERS", "1"))
