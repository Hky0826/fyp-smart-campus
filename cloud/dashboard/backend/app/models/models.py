...
class UploadedDocument(Base):
    ...
    access_level = Column(String(50), default="VISITOR", nullable=True)
    allowed_roles = Column(JSON, nullable=True, default=lambda: ["VISITOR"])
    category = Column(Enum("ACADEMIC", "ADMISSIONS", "FEES_SCHOLARSHIPS", "FACILITIES", "POLICIES", "STUDENT_LIFE", "RESEARCH", "INTERNATIONAL", "GENERAL"), default="GENERAL", nullable=False)
    faculty_code = Column(String(32), nullable=True)
    target_audience = Column(Enum("UNDERGRADUATE", "POSTGRADUATE", "INTERNATIONAL", "STAFF", "STUDENT", "ALL"), default="ALL", nullable=False)
    validity_year = Column(Integer, nullable=True)
...
class DocumentChunk(Base):
    ...
    access_level = Column(String(50), default="VISITOR", nullable=True)
    allowed_roles = Column(JSON, nullable=True, default=lambda: ["VISITOR"])
    chunk_type = Column(Enum("SUMMARY", "DETAIL", "TABLE", "FAQ", "PARENT"), default="DETAIL", nullable=False)
    parent_chunk_id = Column(Integer, ForeignKey("document_chunks.chunk_id", ondelete="CASCADE"), nullable=True)
    ...
    parent_chunk = relationship("DocumentChunk", remote_side=[chunk_id], backref="child_chunks")
